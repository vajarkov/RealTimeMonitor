$(function () {
    ...
    var ViewModel = function () {
        var self = this; // Whether we're connected or not 
        self.connected = ko.observable(false); // Collection of machines that are connected 
        self.machines = ko.observableArray();
    }; // Instantiate the viewmodel.. 

    var vm = new ViewModel(); // .. and bind it to the view 
    ko.applyBindings(vm, $("#computerInfo")[0]);//document.getElementById("computerInfo"));//$("#computerInfo")[0]); // Get a reference to our hub 

    var hub = $.connection.cpuInfo; // Add a handler to receive updates from the server 

    hub.client.cpuInfoMessage = function (machineName, processor, memUsage, memTotal) {
        var machine = {
            machineName: machineName,
            processor: processor.toFixed(0),
