...
const solopt_t solopt_default = { /* defaults solution output options */
	...
	void RTKLib::StartServerThread(void) {
	...
	// confirm overwrite --------------------------------------------------------
	int RTKLib::ConfOverwrite(const char* path)
	{
		char* s;
		FILE* fp;
		int itype[] = {
	...
	double RTKLib::getpos(int num_rov, int num_pos) {
		rtksvrlock(&rtksvr);
		double pos = 0.0;
