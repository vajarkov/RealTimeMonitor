...
//#endif
    ...
    *-----------------------------------------------------------------------------*/
    int  CommonRTK::getbitu(array<unsigned char>^ buff, int pos, int len)
    {
        unsigned int bits = 0;
        int i;
    ...
    /*int  CommonRTK::getbitu(unsigned char* buff, int pos, int len)
    {
        unsigned int bits = 0;
        int i;
        for (i = pos; i < pos + len; i++) bits = (bits << 1) + ((buff[i / 8] >> (7 - i % 8)) & 1u);
    ...
    int CommonRTK::getbits(array<unsigned char>^ buff, int pos, int len)//const unsigned char* buff, int pos, int len)
    {
    ...
    *-----------------------------------------------------------------------------*/
    ...
    int DecodeRTCM::decoderaw(){//unsigned char svr_bytes[], int index){ //rtksvr_t* svr, int index) {
        ...
        for (i = 0; i < 1024; i++) {
            ...
            ret = input_rtcm3(RTK_SVR_DATA::tcp_buff[i]);// svr->rtcm + index, svr_bytes[i]);
                /*obs = &svr->rtcm[index].obs;
                nav = &svr->rtcm[index].nav;
                sat = svr->rtcm[index].ephsat;*/
            //}
    ...
    *-----------------------------------------------------------------------------*/
    unsigned int CommonRTK::rtk_crc24q(int len)//const unsigned char* buff, int len)
    {
        unsigned int crc = 0;
        int i;
        ...
        for (i = 0; i < len; i++) crc = ((crc << 8) & 0xFFFFFF) ^ tbl_CRC24Q[(crc >> 16) ^ RTK_SVR_DATA::nbuff[i]];
        return crc;
    }
    ...
    *
    *-----------------------------------------------------------------------------*/
    int DecodeRTCM::input_rtcm3(unsigned char data){// rtcm_t* rtcm, unsigned char data) {
        ////trace(5, "input_rtcm3: data=%02x\n", data);
        //int nbyte = 0;
        /* synchronize frame */
        //if (rtcm->nbyte == 0) {
        if(RTK_SVR_DATA::nbyte==0){
            if (data != RTCM3PREAMB) {
                return 0;
            }
            //rtcm->buff[rtcm->nbyte++] = data;
            RTK_SVR_DATA::nbuff[RTK_SVR_DATA::nbyte++] = data;
            return 0;
        }
        RTK_SVR_DATA::nbuff[RTK_SVR_DATA::nbyte++] = data;
        //rtcm->buff[rtcm->nbyte++] = data;
        //const unsigned char* buff = @(RTK_SVR_DATA::nbuff[RTK_SVR_DATA::nbyte]);
        if (RTK_SVR_DATA::nbyte == 3) {
            RTK_SVR_DATA::len = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, 14, 10) + 3; /* length without parity */
        }
        //if (rtcm->nbyte < 3 || rtcm->nbyte < rtcm->len + 3) return 0;
        if (RTK_SVR_DATA::nbyte < 3 || RTK_SVR_DATA::nbyte < RTK_SVR_DATA::len + 3) {
            return 0;
        }
        RTK_SVR_DATA::nbyte = 0;
            //rtcm->nbyte = 0;

        /* check parity */
        //const unsigned char* p_buff = reinterpret_cast<const unsigned char*>(&(RTK_SVR_DATA::nbuff));
        //if (CommonRTK::rtk_crc24q(rtcm->buff, rtcm->len) != CommonRTK::getbitu(rtcm->buff, rtcm->len * 8, 24)) {
        if(CommonRTK::rtk_crc24q(RTK_SVR_DATA::len) != CommonRTK::getbitu(RTK_SVR_DATA::nbuff, RTK_SVR_DATA::len * 8, 24)){
    ...
    double DecodeRTCM::getbitg(array< unsigned char>^ buff, int pos, int len) ///const unsigned char* buff, int pos, int len)
    {
    ...
    /*double DecodeRTCM::getbits_38(unsigned char* buff, int pos)
    {
        return (double)CommonRTK::getbits(buff, pos, 32) * 64.0 + CommonRTK::getbitu(buff, pos + 32, 6);
    }*/

    /* get signed 38bit field ----------------------------------------------------*/
    double DecodeRTCM::getbits_38(array<unsigned char>^ buff, int pos)//const unsigned char* buff, int pos)
    {
        return (double)CommonRTK::getbits(buff, pos, 32) * 64.0 + CommonRTK::getbitu(buff, pos + 32, 6);
    }
    ...
    int DecodeRTCM::decode_type1005()//rtcm_t* rtcm)
    {
        ...
        if (i + 140 == RTK_SVR_DATA::len * 8) {
            staid = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 12); i += 12;//CommonRTK::getbitu(rtcm->buff, i, 12); i += 12;
            itrf = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 6); i += 6 + 4;//CommonRTK::getbitu(rtcm->buff, i, 6); i += 6 + 4;
            rr[0] = DecodeRTCM::getbits_38(RTK_SVR_DATA::nbuff, i); i += 38 + 2;//getbits_38(rtcm->buff, i); i += 38 + 2;
            rr[1] = DecodeRTCM::getbits_38(RTK_SVR_DATA::nbuff, i); i += 38 + 2;//getbits_38(rtcm->buff, i); i += 38 + 2;
            rr[2] = DecodeRTCM::getbits_38(RTK_SVR_DATA::nbuff, i);//getbits_38(rtcm->buff, i);
        }
        else {
    ...
    int DecodeRTCM::decode_type1006()//rtcm_t* rtcm)
    {
        ...
        if (i + 156 <= RTK_SVR_DATA::len * 8) {
            staid = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 12); i += 12;
            //staid = CommonRTK::getbitu(rtcm->buff, i, 12); i += 12;
            itrf = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 6); i += 6 + 4;
            //itrf = CommonRTK::getbitu(rtcm->buff, i, 6); i += 6 + 4;
            rr[0] = DecodeRTCM::getbits_38(RTK_SVR_DATA::nbuff, i); i += 38 + 2;
            //rr[0] = getbits_38(&RTK_SVR_DATA::nbuff[RTK_SVR_DATA::nbyte], i); i += 38 + 2;
            rr[1] = DecodeRTCM::getbits_38(RTK_SVR_DATA::nbuff, i); i += 38 + 2;
            //rr[1] = getbits_38(rtcm->buff, i); i += 38 + 2;
            rr[2] = DecodeRTCM::getbits_38(RTK_SVR_DATA::nbuff, i); i += 38;
            //rr[2] = getbits_38(rtcm->buff, i); i += 38;
            anth = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 16);
    ...
    int DecodeRTCM::decode_type1020(rtcm_t* rtcm)
    {
        ...
        //if (i + 348 <= rtcm->len * 8) {
        //    prn = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 6);           i += 6;
        //    geph.frq = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 5) - 7;         i += 5 + 2 + 2;
        //    tk_h = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 5);           i += 5;
        //    tk_m = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 6);           i += 6;
        //    tk_s = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 1) * 30.0;      i += 1;
        //    bn = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 1);           i += 1 + 1;
        //    tb = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, i, 7);           i += 7;
        //    geph.vel[0] = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 24) * P2_20 * 1E3; i += 24;
        //    geph.pos[0] = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 27) * P2_11 * 1E3; i += 27;
        //    geph.acc[0] = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 5) * P2_30 * 1E3; i += 5;
        //    geph.vel[1] = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 24) * P2_20 * 1E3; i += 24;
        //    geph.pos[1] = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 27) * P2_11 * 1E3; i += 27;
        //    geph.acc[1] = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 5) * P2_30 * 1E3; i += 5;
        //    geph.vel[2] = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 24) * P2_20 * 1E3; i += 24;
        //    geph.pos[2] = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 27) * P2_11 * 1E3; i += 27;
        //    geph.acc[2] = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 5) * P2_30 * 1E3; i += 5 + 1;
        //    geph.gamn = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 11) * P2_40;     i += 11 + 3;
        //    geph.taun = CommonRTK::getbitg(RTK_SVR_DATA::nbuff, i, 22) * P2_30;
        //}
        //else {
    ...
    int DecodeRTCM::decode_rtcm3()//rtcm_t* rtcm)
    {
        ...
        int ret = 0, week;
        int type = CommonRTK::getbitu(RTK_SVR_DATA::nbuff, 24, 12);
        ...
        case 1005: ret = DecodeRTCM::decode_type1005(); break;
        case 1006: ret = DecodeRTCM::decode_type1006(); break;
